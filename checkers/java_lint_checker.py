...
def JavaLint(files):
    ...
    if len(target_files) > 0:
        ...
        for rule in only_check_rulesets:
            check_rule_path = os.path.join(Env.JAVA_LINT_CONFIG_PATH,f'rulesets/only_check/{rule}') 
            cmd = (
        ...
        if not failure_flag:
            ...
            for rule in forbidden_rulesets:
                forbidden_rule_path = os.path.join(Env.JAVA_LINT_CONFIG_PATH,f'rulesets/forbidden/{rule}')
                cmd = (
                    f'{pmd_dir}./run.sh pmd -d {" ".join(target_files)} -f text -R '
