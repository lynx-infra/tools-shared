...
from cocoapods.utils import (check_call, convert_git_url_to_http, create_temp_dir, get_full_commit_id, is_git_root)
...
class GitDownloader(Downloader):
    ...
    def download(
        ...
        logging.info(f'Fetch git repository {url if logging.DEBUG else url} to {target_dir}')

        target_dir = target_dir or os.path.join(root_dir, url.rsplit('/')[-1].split('.')[0])
        ...
        logging.debug(f'Fetch git repository {url if logging.DEBUG else url} in {source_dir}')
        ...
        if 'commit' in source:
            ref_spec = source["commit"] if len(source["commit"]) == 40 else get_full_commit_id(source["commit"], url)
            ...
            output = subprocess.check_output(cmd, shell=True, cwd=source_dir).decode('utf-8')
