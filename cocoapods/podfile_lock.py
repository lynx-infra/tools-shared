...
def create_source(repo):
    if isinstance(repo, dict) and ":path" in repo:
        ...
        return GitSource()
    elif isinstance(repo, str) and repo.startswith('git@code.byted.org') or repo.startswith('file://'):
...
class PodfileLock:
    ...
    def __init__(self, file_path, cache_dir=None, vmsdk_version=None):
        ...
        for item in content.get('PODS', []):
            name, version = re.match(NAME_VERSION_PATTERN, (
            ...
            if version != pod.version:
                raise Exception(f'version conflicts: existing: {pod.version} new: {version}')
        ...
        for pod_name, external_source in content.get('EXTERNAL SOURCES', {}).items():
            source_root_dir = os.path.join(os.path.dirname(self.file_path), external_source[":path"])
