...
class Specification:
    def __init__(self, pod, parent, spec_content, test_specification=False, app_specification=False):
        self.parent = parent
        self.pod = pod
        ...
        self.subspecs = [
                            Specification(self.pod, self, content, False, False) for content in subspecs
                        ] + [
                            Specification(self.pod, self, content, False, True) for content in appspecs
                        ] + [
                            Specification(self.pod, self, content, True, False) for content in testspecs
                        ]
        self.requires = []
        self.required_by = []
        ...
        for subspec in self.subspecs:
            for key, value in self._attrs.items():
                if key == 'pod_target_xcconfig':
                    subspec.set_attr(key, {**value, **getattr(subspec, 'pod_target_xcconfig', {})})
    ...
    def glob_files_with_patterns(self, file_patterns, include_dirs=False):
        ...
        for pattern in file_patterns:
            ...
            file_list += self._files_accessor.glob_files(pattern, include_dirs=include_dirs)
        return file_list
    ...
    def glob_files_for_attr(self, attr, platform, include_dirs=False):
        ...
        if attrs is None:
            return []
        return self.glob_files_with_patterns(attrs.get(attr, []), include_dirs=include_dirs)

    def convert_xcconfig_for_platform(self, target, xcconfig, platform=None, rebase=None):
        definitions = xcconfig.get('GCC_PREPROCESSOR_DEFINITIONS')
        if definitions:
            definitions = definitions.replace("$(inherited)", "")
            definitions = [f'{k}={v}' if v else k for k, v in parse_shell_style_vars(definitions).items()]
            target.add_defines(definitions)
        header_search_paths = \
            xcconfig.get('HEADER_SEARCH_PATHS', "") + " " + xcconfig.get('USER_HEADER_SEARCH_PATHS', "")
        if header_search_paths:
            header_search_paths = header_search_paths.replace("$(inherited)", "")
            target.add_include_dirs([
                os.path.relpath(
                    expandvars(p), rebase or self.pod.target_dir
    ...
    def convert_attrs_for_platform(self, target, attrs, platform, root_dir, rebase=None):
        compiler_flags = attrs.get('compiler_flags', [])
        for f in compiler_flags if isinstance(compiler_flags, list) else [compiler_flags]:
            if f.startswith('-I$'):
                f = expandvars(f)
        ...
        if frameworks:
            ...
            ] if isinstance(frameworks, list) else f'{frameworks}.framework'
            target.add_frameworks(frameworks, condition=platform)

        target.add_libs(self._files_accessor.get_libraries(platform, rebase=rebase), condition=platform)
        vendored_libraries = self._files_accessor.get_vendored_libraries(platform, rebase=rebase)
    ...
    def all_header_mappings(self):
        ...
        for platform in [None, 'ios', 'osx']:
            ...
            private_mappings += [
                (target, source) for target, source in self.get_header_mappings(platform, header_files).items()
            ]
            public_mappings += [
                (target, source) for target, source in self.get_header_mappings(platform, public_header_files).items()
            ]
    ...
    def collect_headers(self, platform):
        ...
        header_dir = os.path.join(
            os.path.dirname(self.pod.target_dir), 'Headers',
        )
    ...
    def convert_vendored_frameworks(self, name, target, platform, rebase):
        vendored_framework_files = self._files_accessor.get_vendored_frameworks(platform, rebase=rebase)
        if not vendored_framework_files:
            return
        frameworks = {f.split('/')[-1].split('.framework')[0]: f for f in vendored_framework_files}
        ...
        for framework_name, framework in frameworks.items():
            ...
            shared_library_target, _ = self.get_or_create_target('shared_library', shared_library_name)
            ...
            bundle_target.add_outputs(
                '{{bundle_resources_dir}}/Frameworks/' + framework_name + '.framework/{{source_file_part}}'
            )
    ...
    def convert_to(self, type_, root_dir, rebase=None):
        ...
        if isinstance(requires_arc, list):
            ...
            arc_source_set, _ = self.get_or_create_target('source_set', arc_source_set_name)
            arc_sources = self._files_accessor.glob_files_for_attr('requires_arc', None, rebase=rebase)
        ...
        for platform in [None, "ios", "osx"]:
            if platform:
                ...
                if pch_path:
                    private_config.add_cflags(
                        (
                            "-include",
                            Rebase(os.path.relpath(pch_path, rebase or self.pod.target_dir), "root_build_dir")
                        ),
                        platform
                    )
            ...
            source_files = self._files_accessor.get_source_files(platform, rebase=rebase)
            target.add_sources(source_files if not arc_sources else [f for f in source_files if f not in arc_sources])
            ...
            if resources and name not in ['lens_ImageVRSR', 'lens_ImageNNSR']:
                ...
                resources_target, _ = self.get_or_create_target('bundle_data', bundle_name)
                resources_target.add_sources(resources)
                resources_target.add_outputs('{{bundle_resources_dir}}/{{source_file_part}}')
                target.add_public_deps(f':{bundle_name}')
            ...
            for spec in self.requires:
                ...
                if dep_name.startswith("Lynx"):
                    ...
                    dep_name = f'//{os.path.relpath(os.path.dirname(rebase or self.pod.target_dir), root_dir)}/' \
            ...
            self.convert_attrs_for_platform(private_config, attrs, platform, root_dir=root_dir, rebase=rebase)
            ...
            private_headers_dir = os.path.join('..', 'Headers', 'Private', self.pod.name)
            private_header_mappings, public_header_mappings = self.collect_headers(platform)
            if private_header_mappings:
                private_config.add_include_dirs(
                    private_headers_dir, condition=platform
                )
            ...
            if public_header_mappings:
                public_config.add_include_dirs(
                    public_headers_dir, condition=platform
                )
                private_config.add_include_dirs(
                    public_headers_dir, condition=platform
                )
