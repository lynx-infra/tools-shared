...
def CheckCommitMessage(message):
  ...
  if IsRevertedCommit(commit_lines):
    ...
    # check title
      ...
      return ERROR_MALFORMED_MESSAGE, f'Malformed title. Title "{commit_lines[0]} not match reg "{reg_str}"'

    if commit_lines[1] != '':
      return ERROR_MALFORMED_MESSAGE, 'No empty lines found between title and summary.'
    ...
    for line in commit_lines[2:]:
      summary += line
      if re.match(r'((issue):\s*([FfMm]-|(\#))(\d)+)|(no-((meego)|(workitem)))', line):
